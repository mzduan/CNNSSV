...
# 计算分裂比对断点左端的混乱度：1000个base的比对情况
def get_left_confusion_splits(ele,ref_dict,query_seq):
...
# 计算分裂比对断点右端的混乱度：1000个base的比对情况
def get_right_confusion_splits(ele,ref_dict,query_seq):
...
def cluster_DEL(sv_list,min_support,min_sv_len,max_sv_len):
    ...
    tmp_del_cluster.append([0,0,'',0,0,0,0])
...
def cluster_INS(sv_list,min_support,min_sv_len,max_sv_len):
    ...
    tmp_ins_cluster.append([0, 0, '', '',0, 0,0,0])
...
def cluster_DUP(sv_list,min_support,min_sv_len,max_sv_len):
    ...
    tmp_dup_cluster.append([0, 0, '', 0, 0,0,0])
...
def cluster_INV(sv_list,min_support,min_sv_len,max_sv_len):
    ...
    tmp_inv_cluster.append([0, 0, '',0,0,0,0])
...
def run_get_breakpoints(aln,min_sv_len,ref_dict,breakpoints):
    ...
    if chro=="" and start==-1 and end==-1:
        ...
        for aln in alns:
            if aln.is_unmapped or aln.mapping_quality < min_map_qual:
                ...
                run_get_breakpoints(aln, min_sv_len, ref_dict, breakpoints)
